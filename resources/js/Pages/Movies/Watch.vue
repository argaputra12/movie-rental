...
const sendUpdateRequest = async () => {
  try {
    const response = await axios.put(`/movies/update-duration/${props.userMovie.id}`, {
    ...
    if (remainingTime.value <= 0) {
      ...
      $toast.open({
        message: "Time is up! Status changed to waiting.",
        type: "error",
      });
...
<template>
  ...
  <div v-if="userMovie === null || userMovie.status !== 'approved'" class="bg-gray-800 absolute inset-0 bg-opacity-90 flex justify-center items-center">
    <div class="bg-white w-1/2 mx-auto mt-20 p-8 rounded-lg">
      <h2 class="text-3xl font-extrabold text-gray-900">Request for movie</h2>
      <p class="text-gray-500 mt-4">You need to request this movie from the admin.</p>
      <div class="mt-8 flex justify-end gap-4">
        <FwbButton color="alternative" @click="goBack">Go back</FwbButton>
        <FwbButton color="default" @click="requestAccess(movie.id)">Request access</FwbButton>
      </div>
    </div>
  </div>
  ...
  <div class="bg-gray-800">
    <div class="max-w-7xl mx-auto py-16 px-4 sm:px-6 lg:py-20 lg:px-8 min-h-screen">
      <iframe :src="movie.video_url" frameborder="0" class="w-full h-[720px]"></iframe>
    </div>
  </div>

  <!-- Timer Display -->
  <div v-if="remainingTime > 0" class="fixed bottom-0 right-0 m-4 bg-black text-white p-2 rounded-md">
